...
export function formatRelativeFromMs(diffMs: number): string {
  ...
  if (safe < hour) {
    ...
    return `about ${mins} min${mins === 1 ? '' : 's'} ago`
  }
  if (safe < day) {
...
export function formatRelativeToMs(diffMs: number): string {
  const safe = Math.max(0, diffMs)
  const minute = 60_000
  const hour = 60 * minute
  const day = 24 * hour

  if (safe < 45_000) return 'in a few seconds'
  if (safe < hour) {
    const mins = Math.round(safe / minute)
    return `in about ${mins} min${mins === 1 ? '' : 's'}`
  }
  if (safe < day) {
    const hrs = Math.round(safe / hour)
    return `in about ${hrs} hr${hrs === 1 ? '' : 's'}`
  }
  const days = Math.round(safe / day)
